...
def get_submenus(menu_id: int, db: Session = Depends(get_db)):
    ...
    set_cache(f"http://127.0.0.1:8000/api/v1/menus/{menu_id}/submenus", submenus)
    return submenus
...
def get_submenu(menu_id: int, submenu_id: int, db: Session = Depends(get_db)):
    ...
    set_cache(f"http://127.0.0.1:8000/api/v1/menus/{menu_id}/submenus/{submenu_id}", db_submenu)
    return db_submenu
...
def update_submenu(menu_id: int, submenu_id: int, subm: SubmenuUpdate, db: Session = Depends(get_db)):
    ...
    if db_submenu is None:
        raise HTTPException(status_code=404, detail="submenu not found")
    updated_submenu = submenu.update_submenu(db=db, submenu=subm, submenu_id=submenu_id)

    set_cache(f"http://127.0.0.1:8000/api/v1/menus/{menu_id}/submenus/{submenu_id}", updated_submenu)
    delete_cache(f"http://127.0.0.1:8000/api/v1/menus/{menu_id}/submenus")
